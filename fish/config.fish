...
function fish_greeting
	...
	if [ $r -lt 10 ]
		...
		set_color cyan
		 ...
		 echo "  [workstation] <create .wallpaper directory and source wallpapers from there>" 
	end
	if [ $r -lt 25 ]
	...
	if [ $r -lt 50 ]
		...
		echo "[intergrate tmux/fish/alacrity] <currently tmux is commented from config.fish>"
	end

	# urgent, so prompt always
	set_color red
	echo "  [arch_installation/wifi] configure an automated way to connect to the internet"
